...
class ModelWriter:
    ...
    def create_network_tokens(self):
        ...
        inflow_rates = {k: f"(np.interp(time,F_in[{c},flow_time) - S[{c}])(1-decay_constant)" for c, k in enumerate(input_ids)}
        ...
        outflow_rates = {k: f"decay_constant" for c, k in enumerate(output_ids)}
